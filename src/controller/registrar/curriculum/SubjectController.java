 ...
 */
@WebServlet("/Registrar/Controller/Registrar/Curriculum/SubjectController")
public class SubjectController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(type.equals("Insert"))
			...
			sql = "Update r_subject Set Subject_Code = '"+ec.encrypt(ec.key, ec.initVector, code)+"',Subject_Description =  '"+ec.encrypt(ec.key, ec.initVector, desc)+"',Subject_Lecture_Units =  "+lecunit +",Subject_Laboratory_Units = "+labunit +" ,Subject_Lecture_Hours = "+lechrs +" ,Subject_Laboratory_Hours = "+labhrs +" ,Subject_Tuition_Hours = "+tuihrs +" ,Subject_Type ='"+ acadtype+"' ,Subject_Group = (select case when '"+subject+"' = '0' then null else (SELECT t2.Subject_ID FROM (select * from r_subject) as t2 WHERE t2.Subject_Code = '"+ec.encrypt(ec.key, ec.initVector, subject) +"') end as grp) ,Subject_Credited_Units = "+credunit+" where Subject_Code = '"+ec.encrypt(ec.key, ec.initVector, latcode)+"' ";
