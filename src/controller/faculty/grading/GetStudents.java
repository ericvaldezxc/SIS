 ...
 */
@WebServlet("/Faculty/Controller/Faculty/Grading/GetStudents")
public class GetStudents extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			sql = "SELECT  *,IFNULL(Students_Grade_Grade,'Not S') GRADE from t_student_taken_curriculum_subject inner join t_student_account on Student_Taken_Curriculum_Subject_StudentAccountID = Student_Account_ID inner join r_student_profile on Student_Account_Student_Profile_ID = Student_Profile_ID inner join r_section on Student_Taken_Curriculum_Subject_SectionID = Section_ID inner join r_subject on Student_Taken_Curriculum_Subject_SubjectID =  Subject_ID inner join r_curriculumitem on CurriculumItem_SubjectID = if(ifnull(Subject_Group,0)=0,Subject_ID,Subject_Group) inner join t_schedule on Schedule_CurriculumItemID = CurriculumItem_ID left join t_students_grade on Students_Grade_StudentTakenCurriculumSubjectID = Student_Taken_Curriculum_Subject_ID left join r_professor on Schedule_ProfessorID = Professor_ID  where Student_Taken_Curriculum_Subject_SemesterID is not null  and Student_Taken_Curriculum_Subject_Taken_Status = 'true' and Section_Code = '"+section+"' and Subject_Code = '"+subjectDrp+"' and Student_Taken_Curriculum_Subject_SemesterID = (SELECT Semester_ID FROM r_semester WHERE Semester_Code = '"+semesterDrp+"') and Student_Taken_Curriculum_Subject_AcademicIYearID = (SELECT Academic_Year_ID FROM `r_academic_year` WHERE Academic_Year_Code = '"+acadyearDrp+"'  )  and if (Schedule_ChildrenID is null ,0,Schedule_ChildrenID) = if(Schedule_ChildrenID is null ,0,Subject_ID) and Schedule_SectionID = Section_ID and Schedule_ProfessorID is not null and Schedule_AcademicYearID = (SELECT myt1.Academic_Year_ID FROM `r_academic_year` as myt1 where myt1.Academic_Year_Active_Flag = 'Present') ";
			//out.print(sql);
