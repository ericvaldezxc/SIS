    ...
    return {
        ...
        init: function () {
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: 'post',
                                url: 'Controller/Registrar/Pre-Enrollment/Announcement',
                                data: {
                                	getval: getval
                                },
            ...
            $('#updateBtn').click(function(e){
            	e.preventDefault();
            	codeTxt = $('#codeTxtUpdate').val();
            	descTxt = $('#descTxtUpdate').val();
                ...
                function (isConfirm) {
                    if (isConfirm) {
                		$.ajax({
        					type:'POST',
        					data:{codeTxt: codeTxt, descTxt: descTxt,type:"Update",latcode:latcode},
        					url:'Controller/Registrar/Pre-Enrollment/Announcement',
        					success: function(result){
                                 swal({
                                     title: "Record Updated!"
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                ...
                                url: 'Controller/Registrar/Pre-Enrollment/Announcement',
                                data: {
                                	codeTxt: codeTxt, descTxt: descTxt,type:"Delete",latcode:latcode
                                },
                                success: function (response) {
                                    swal({
            ...
            $('#editable-sample').on('click','a.edit', function (e) {
                ...
                var getcode = $(this).data('id');
                latcode = getcode;
                var getdesc = $(this).closest('tr').children('td:first').next().text();
                var nRow = $(this).parents('tr')[0];
                
        		$('#codeTxtUpdate').val($(this).closest('tr').children('td:first').text())
        		$('#descTxtUpdate').val(getdesc)
